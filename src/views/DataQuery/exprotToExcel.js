...
function transform(tableData, name, callback) { // table为表格数据,name为导出文件名,//callback为导出完毕回调,方便你知道导出完成了(可根据自己需求决定是否需要)
  ...
  let tableInnerHTML = ''
  const headerData = ['日期', '物料名称', '消耗量']
  const bodyData = tableData // 这里对应是表格数据，我们只需要传过来即可
  ...
  bodyData.forEach(item => {
    tableInnerHTML += '<tr>'
    tableInnerHTML += "<td align='center' style='height:66px; border:solid;mso-number-format:`\@`'>" + (item.date || '') + '</td>'
    tableInnerHTML += "<td align='center' style='height:66px; border:solid'>" + (item.name || '') + '</td>'
    tableInnerHTML += "<td align='center' style='height:66px; border:solid'>" + (item.num || '') + '</td>'
    tableInnerHTML += '</tr>'
  })
